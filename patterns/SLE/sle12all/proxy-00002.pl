...
# Title:       Proxy active but not configured
# Description: Active proxy even though /etc/sysconfig/proxy is not configured for proxy
...
@PATTERN_RESULTS = (
	...
	"META_LINK_TID=http://www.suse.com/support/kb/doc.php?id=7006744"
);
	...
	if ( getProxyActive() ) {
		...
		if ( getProxyConfigured() ) {
			...
			while ( ($KEY) = each(%PROXY_ACTIVE) ) {
				if ( ! defined $PROXY_CONFIG{$KEY} ) {
					SDP::Core::printDebug(" PUSH  FAILED", "$KEY");
					push(@PROXY_FAILED, $KEY);
				}
			}
			if ( $#PROXY_FAILED >= 0 ) {
				SDP::Core::updateStatus(STATUS_CRITICAL, "Active Proxies with NO Configuration: @PROXY_FAILED");
			} else {
				my @PROXIES = keys (%PROXY_ACTIVE);
				SDP::Core::updateStatus(STATUS_ERROR, "All Active Proxies are Configured: @PROXIES");
			} 
		} else {	
			@PROXY_FAILED = keys (%PROXY_ACTIVE);
			SDP::Core::updateStatus(STATUS_CRITICAL, "All Active Proxies with NO Configuration: @PROXY_FAILED");
		}
	} else {
